...
# Use find to handle filenames with spaces — check all zips and csvs
while IFS= read -r -d '' file; do
  ...
  # RingConn — Data Export-Pete-*.zip with 3 CSVs inside
  elif echo "$lower" | grep -qE '^data export.*\.zip$'; then
    move_to_inbox "$file"

  # Withings — data_PET_ numeric zips or export_ prefix
  elif echo "$lower" | grep -qE '^(export_|data_export|data_pet_|withings|healthmate).*\.(zip)$'; then
    move_to_inbox "$file"
