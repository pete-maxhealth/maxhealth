...
SOURCE_FIELDS = {
    'withings': ['weight', 'bmi', 'fat_pct', 'muscle_pct', 'hrv', 'spo2',
                 'sleep_duration', 'sleep_deep', 'sleep_light', 'sleep_rem',
                 'sleep_wake', 'steps', 'hr_avg', 'hr_min', 'hr_max'],
    'ringconn': ['weight', 'hrv', 'spo2', 'sleep_duration', 'sleep_deep',
                 'sleep_light', 'sleep_rem', 'sleep_wake', 'steps',
                 'hr_avg', 'hr_min', 'hr_max'],
    'garmin':   ['steps', 'distance_m', 'calories_active', 'sleep_duration',
                 'sleep_deep', 'sleep_light', 'sleep_rem', 'sleep_wake',
                 'hr_avg', 'hr_min', 'hr_max', 'hrv'],
    'amazfit':  ['steps', 'distance_m', 'calories_active', 'sleep_duration',
                 'sleep_deep', 'sleep_light', 'sleep_rem', 'sleep_wake',
                 'hr_avg', 'hr_min', 'hr_max', 'weight', 'bmi',
                 'fat_pct', 'muscle_pct'],
}

# Map metric category → combined.csv fields
METRIC_FIELDS = {
    'weight': ['weight', 'bmi', 'fat_pct', 'muscle_pct'],
    'hrv':    ['hrv'],
    'sleep':  ['sleep_duration', 'sleep_deep', 'sleep_light', 'sleep_rem', 'sleep_wake'],
    'steps':  ['steps', 'distance_m', 'calories_active'],
    'spo2':   ['spo2'],
...
def backup_files():
    """Copy combined.csv and nutrition.csv to backup dir with timestamp."""
    os.makedirs(BACKUP_DIR, exist_ok=True)
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    backed_up = []

    for src_path, name in [(COMBINED, 'combined'), (NUTRITION, 'nutrition')]:
        if os.path.exists(src_path):
            dst = os.path.join(BACKUP_DIR, f"{name}_{ts}.csv")
            shutil.copy2(src_path, dst)
            backed_up.append(dst)

    # Trim to MAX_BACKUPS per file type
    for prefix in ['combined', 'nutrition']:
...
def write_combined(rows_by_date, dry_run=False):
...
def merge_with_precedence(existing_rows, new_rows_by_source, precedence):
    ...
    for date in all_dates:
        ...
        # For each metric category, apply precedence
        for metric, prio_sources in precedence.items():
            fields = METRIC_FIELDS.get(metric, [])

            # Check if any field in this metric already has a value in existing row
            already_filled = any(row.get(f) for f in fields)
            ...
            for source in prio_sources:
                ...
                src_row = source_rows[date]
                has_data = any(src_row.get(f) for f in fields)
                if not has_data:
                    continue
                ...
                if not already_filled:
                    ...
                    # Fill individual missing fields (gap filling)
                    for f in fields:
                        if not row.get(f) and src_row.get(f):
                            row[f] = src_row[f]
                            sources_used.add(source)
                break  # highest priority source with data wins
...
def run_extractor(device, inbox, password=None, dry_run=False):
    ...
    if not os.path.exists(extractor_path):
        log(device, 'extract', 'error', f"No extractor found at {extractor_path}")
        return {}
...
def main():
    ...
    # Write output
    write_combined(merged, dry_run=args.dry_run)
