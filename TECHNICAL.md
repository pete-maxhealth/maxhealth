...
| `mh_mm_selected_metrics` | Compare Metrics — which metric keys are currently overlaid on the chart |
| `mh_mm_period` | *(in-memory only — not persisted)* Compare Metrics' own 7D/30D/90D/All, separate from the Trends page filter |
| `mh_mm_legend_order` / `mh_mm_legend_hidden` | Compare Metrics — per-card reorder/hide within the legend, scoped to selected metrics |
| `mh_mm_section_order` / `mh_mm_section_hidden` | Compare Metrics — reorder/hide of the three whole sections (Chart / Day-Period Info / Legend) |
| `mh_trend_cards_order` / `mh_trend_cards_hidden` | Trends page — reorder/hide of all 15 metric cards |
| `mh_chart_styles` | Trends cards — per-metric chart style (line/bar/point) |
| `mh_mm_chart_styles` | Compare Metrics — per-metric chart style, kept separate from `mh_chart_styles` so a metric's overlay style and its own card's style don't collide |
| `mh_bodycomp_unit` | Bone Mass / Hydration cards — which unit (kg or %) drives the big number, stats, and chart; the other unit still always shows on a secondary line regardless |
| `mh_treatment_defs` | Treatment Tracking — user-defined treatments (id/name/explanation/active), same shape as `mh_symptom_defs` |
| `mh_treatment_log` | Treatment Tracking — a growing array of real timestamped sessions (`{id, treatmentId, date, time, note}`), NOT a daily-reset snapshot like symptom/supplement logs — this is what makes before/during/after-session reporting possible |
| `mh_custom_tag_parents` | Occasion Tags — learned map of custom tag text → parent category (Treatment/Exercise/none), asked once per distinct tag text and never re-prompted |
| `mh_carer_email` | Settings → Carer & Clinician — pre-fills the recipient on Treatment Sessions' "Email this" and similar report-sending features; never sent anywhere automatically |
| `mh_saved_queries` | Shared by the Reports Query Builder and Treatment Sessions "Save this query" — a `type` field (`'treatment'` vs the original range-query shape) lets one list and one Run button handle both without either guessing at the other's fields |
...
| v3.10.564-584 | — | Not individually documented — no summary carried into this session |
| v3.10.585-594 | 19 | Compare Metrics multi-line overlay fixed (per-dataset pointRadius, was silently hiding any metric with data gaps), single-metric drill chart and Your Journey both fixed for horizontal scroll (the real blocker was a `preventDefault` touchmove handler, not touch-action CSS) and given tap-to-open info panels instead of floating tooltips, reorder+hide added at three levels (legend cards, compare-metrics sections, all 15 Trends metric cards) with a persisted-order pattern matching the dashboard's own, a same-session lockout bug found and fixed (hide control living inside the thing it hides — now a standing restore-strip pattern), old "Full Charts" section removed and substantially rebuilt inline into existing cards plus 6 new cards for metrics that never had one, `water` added to `DRILL_CONFIG` (had a UI element with no backing entry), Trends page reordered (Analysis period pinned to the top and made non-hideable, Wellness/Journey/Patterns grouped beneath it) |
| v3.10.595-598 | 19 | Chart style picker (line/bar/point) on all 15 Trends cards and per-metric in Compare Metrics (true mixed chart), remembered per tile. Zepp-inspired visual polish kept in MaxedHealth's own palette: two globally-registered Chart.js plugins (today-highlight band, last-point data labels) apply to every chart automatically; threshold-aware coloring on HRV/SpO2; new horizontal gauge component on HR/HRV/SpO2/Steps/Sleep/Ketosis Adherence and the deep-dive screen. Bone Mass/Hydration investigated properly via the Body Comp Debug panel against real cache data — resolved as correct data (both genuinely improving as % of body weight) presented misleadingly (kg alone falls when a ~24kg total weight loss outpaces the % improvement); both cards now show kg and % simultaneously with a toggle for which drives the chart, defaulting to % |
| v3.10.599-627 | 20 | Real notification-delivery bug fixed (all 4 types marked "fired" on attempt, not success); fat-sanitization overwriting deliberate zeros fixed (incl. the specific alcohol-calories case); device-precedence imports were permanently blocked after the first fill, now update on every fresh import; export button and "Redo setup wizard" both fixed (real bugs, not perception). Patterns gained SPC-style early-warning checks (`checkSPCBreach`, Western Electric/Nelson rules) for Calories/Weight plus 6 new cross-metric findings. Library gained Auto-Categorize (macro-ratio based, review queue for the rest) and an exercise-name duplicate finder. New Treatment Tracking system (defs + real timestamped sessions, built on the Symptom pattern, kept separate from the pre-existing keyword-based "Treatment Analysis"), wired into Compare Metrics, a new "Treatment Sessions" before/during/after report, chart image export, and an audience-agnostic "Custom" export mode |
| v3.10.628-658 | 21 | **Root-cause fixes, not symptom patches**: `trendsWindow` (a Trends-page-only variable, defaulting to 'today') was silently limiting Compare Metrics and the Deep Dive view to a single day regardless of their own period selection — found via a precise repro, fixed with a `forceFullHistory` bypass; a `state.history` object was being mutated directly instead of cloned, silently corrupting real saved data on every render; 5 color-collision groups across the 16 metric pills. Compare Metrics gained Custom date range, category-grouped pills (Treatment/Exercise/Vitals & Sleep/Nutrition/Body Composition/Symptoms), and a 📦 Full Report Package (chart + CSV + AI interpretation bundled via the Web Share API, with an honest mailto fallback). Treatment Tracking extended substantially: past-day session logging, session-level and general per-tag notes, a Today-dashboard card, and Reports → Treatment Sessions rebuilt for multi-select treatments × metrics with CSV export and AI interpretation. Three separate, real voice-input bugs found and fixed (auto-send on "done", trailing-phrase detection on long utterances, live input-box updating — the actual cause of "the arrow does nothing"). Meal logging: numbered-suggestion context loss fixed and then hardened against the AI calling a different tool instead of logging the selection; the actual root cause of a persistent "keto X never logs" report turned out to be a deterministic keyword shortcut hijacking short "keto"/"ketosis" messages into a status readout, not a logging bug at all. Recipe `useCount` resolved through four real rounds of debug-trace-driven fixes: centralized into `logFoods()` as one choke point, a missing reverse-direction name check, and finally a genuinely different ingredient-set-overlap comparison for the `logSuggestedCombo` branch that never referenced a recipe by name at all. Period selectors standardized (7D/30D/All/Custom) across Trends, Reports, and Compare Metrics, with Today kept as a Trends-only addition. Settings reorganized (Data & Backup moved into the renamed "Data" tab). Full site-search audit by script found and fixed the entire Reports tab plus 17 other sections being unindexed, and 7 wrap-id mismatches (3 pre-existing). New PWA update-available banner, prompted by a device found stuck 3 versions behind with no way to know |
| v3.10.658-729 | 22 | **Two systemic bug classes, found by pulling one thread until it was actually exhausted, not patched-and-moved-on**: (1) gross-vs-net carbs — `getEffectiveCarbs()`/`getEffectiveCarbsForDay()` existed and was correct, but ~20 separate places (Reports summary, Weekly Summary, the clinical report, `generateAIBrief`, the AI daily-insight prompt, several Trends charts, a weekly push notification, the carer-link export) still read raw gross carbs for real adherence %/averages, not just display — 4 of those had already had their *label* condition-fixed in an earlier pass while the number feeding it was still wrong, a pattern only caught by re-checking every instance rather than trusting the earlier fix. (2) condition-generalized "ketosis" wording — new shared `isKetogenicCondition()` (reuses `CONDITION_META`'s protocol text) replaced several inconsistent ad-hoc `isGBMorEpilepsy` checks (one covered only 2 of 5 genuinely ketogenic conditions) across the main dashboard badge, its tooltip, streak-milestone celebrations (two full parallel message sets), `getKetosisStatus()`, Settings' fat-target advice (3 hand-duplicated copies centralized into `updateFatTargetAdvice()`), and the dashboard's fat-ratio smart-alert (hidden entirely for non-keto conditions rather than reframed, since a 65% fat floor isn't a meaningful concept outside a ketogenic protocol). Saved Prompts gained scope tagging (Today/History/Both, keyword-suggested as you type, one-tap batch classify for pre-existing prompts) after a real routing gap: `state.history` never included today (only completed days), so a history-needing question run from the Log tab could never be answered regardless of which list it came from — new `_historyIncludingToday()` merges today's live totals into Reports' context, making History a genuine superset. New Balance My Macros (needs-driven, deliberately unfiltered by the category-override snack-suppression logic that correctly applies to Next Meal Idea). Root-caused a library item silently failing to scale: portion-parsing required an explicit unit suffix, a bare numeric portion always fell back to a 100g guess — independently hand-copied across 8 call sites, centralized into `_parsePortionGrams()`. Category comma-splitting never worked (`addItemCategory`/`addCategoryToWord` both took the whole input as one literal string); wearable CSV import never mapped a `fat_g` column at all despite the app's own export schema expecting one; `_fuzzyMatchSupplement()` compared incompatible units (char length vs word-hit count) in the same comparison, letting a weak exact match block a better fuzzy one. |
| v3.10.730-736 | 22 | **Category data loss on "Save only", found and fixed across several rounds**: `saveLibraryOnlyFromPreview()` and the separate fuzzy-name-match confirm path both did a full wholesale replace of the library entry (`lib[idx] = entry`) rather than an update — since the replacement only ever carried `.categories` when something was freshly typed that specific time, leaving the field blank (the normal re-save case) silently wiped every category the item already had. Fixed by updating fields in place and merging categories through a new shared `mergeCategoriesIntoLibraryItem()` — the same seed-from-auto-detected-if-empty, append-don't-replace logic the Library edit form's `addItemCategory()` already used correctly, which was itself refactored onto the same shared helper on request rather than keeping a separate copy. The meal-preview screen now shows an "Already saved as: …" line for existing library items so both screens agree on current state. A second, related bug found while re-testing the first: adding/removing a category chip in the Library edit form called a full `renderLibrary()`, which rebuilt the entire list and silently closed that item's own edit form (wiped its `.open` CSS class) after every single tap — fixed by patching just the affected item's chip container in place via a new shared `renderCategoryChipsHtml()`. Two remaining gaps closed to finish the thread: the ✏️ per-item edit form (distinct from the shared bubble-level field) had no categories input at all, now merges its own `_categoriesOverride` in alongside the shared field at save time; a brand-new item with both fields left blank previously saved with no `.categories` stored at all (display-only auto-detection) — now seeded explicitly via `getItemCategories()` at creation. Two rounds of debug-trace instrumentation (`mh_addcat_trace`, `mh_saveonly_cat_trace`) and a try/catch around `saveLibraryOnlyFromPreview()` (after a report of literally silent failure) were added along the way and left in place under Settings → Debug. |
| v3.10.737-743 | 22 | **Debug-tools cleanup, one more gross-vs-net carbs gap, a stuck read-aloud button, and supplement tablet counts/reordering.** Retired the polyols-pipeline and impact-box debug traces (both long-fixed, long-stable) and added **📋 Copy Diagnostics for Claude** — a Settings section listing every remaining diagnostic source with a live entry count, pre-ticking whichever have something to contribute, bundling the selection into one paste-ready block via each source's text-builder function (extracted from the existing render functions so nothing's duplicated between the individual "View X trace" buttons and the combined view). All debug sections now share a consistent icon and sit together as one group. Found one more spot the earlier systemic gross-vs-net carbs sweep missed: the Deep Ketosis smart-alert banner was reading raw `totals.carbs` for both its displayed figure and its ceiling comparisons — fixed to use `getEffectiveCarbs()` like everywhere else. Added "was X, Yg after polyols" display to three more spots (Today's Log, Edit Entry form, the chat "LOGGED ✓" bubble) that showed only the raw carbs number with no indication of whether polyols had been subtracted. Fixed the GBM Monthly Summary's stuck read-aloud button — it read text via `.innerText`, which silently returns empty for a hidden/not-yet-laid-out element (unlike `.textContent`), and speaking an empty utterance could leave the button stuck on "stop"; fixed with the same `innerText`-with-fallback pattern `copyElementText` already used, plus a general empty-text guard added to `toggleSpeakText()` itself. Added an optional Tablets field to supplements (separate from the mg dose) and made time-of-day lists reorderable — Manage Supplements is now grouped by period like the Today tab, with ▲▼ buttons; ordering is stored per-period (`s.order[period]`) rather than as one global position, since a supplement appearing in more than one period isn't required to sit in the same relative spot in each. |
...
**Compare Metrics and Trends cards (Phase 19) add two more independent implementations**, same core pattern (persisted order array + `appendChild` reorder + the same already-correct-order check), but neither uses `makeContainerReorderable` — both needed hide as well as reorder, which the generic system doesn't support:
- `applyMMSectionOrderAndVisibility()` — the three compare-metrics sections (Chart / Day-Period Info / Legend). Order: `mh_mm_section_order`. Hidden: `mh_mm_section_hidden`.
- `applyTrendCardsOrderAndVisibility()` — all 15 Trends metric cards. Order: `mh_trend_cards_order`. Hidden: `mh_trend_cards_hidden`. Called from the end of `renderTrends()`, after every `showCard()` call, not before — a card's data-driven visibility must win first, and a user hide only overrides it back to `none` afterward, or it'd just get overwritten.

## Chart style picker, threshold coloring, and gauges (line/bar/point + Zepp-inspired polish)

`applyChartStyle(dataset, style)` is the one place a chart dataset's `type` gets set from a stored preference (`getChartStyle`/`setChartStyle`, keyed `mh_chart_styles` for Trends cards and `mh_mm_chart_styles` for Compare Metrics) - 'point' is a line dataset with `showLine:false` and enlarged points, not real Chart.js `scatter`, since every chart here uses date-label category axes and scatter needs numeric x. Reference/target lines are never passed through this function - they stay fixed dashed guides regardless of the real data's style.

`applyThresholdStyling(dataset, rawVals, ref, baseColor)` layers threshold-aware coloring on top of a style-applied dataset, only when the metric has a real `ref` band in `DRILL_CONFIG`. Line style colours by *segment* (Chart.js's `segment.borderColor` callback, keyed off each segment's end-point value); bar/point style colours by point. Currently wired into HRV and SpO2 only - Steps kept its own pre-existing green/blue goal-based coloring since "more steps" isn't an out-of-range concept the same way; Sleep/Ketosis weren't retrofitted (stacked/binary data doesn't map cleanly onto a continuous-value threshold).

`renderThresholdGauge(value, ref, unit, color)` renders the horizontal segmented gauge (amber zones outside `ref`, a marker at the real value) as plain HTML/CSS, not a Chart.js chart - used on HR, HRV, SpO2, Steps, Sleep, Ketosis Adherence, and the deep-dive screen, everywhere a real ref band exists to draw zones from.

`mhTodayHighlightPlugin` and `mhDataLabelPlugin` are registered once, globally, via `Chart.register(...)` near `CHART_DEFAULTS` - every `new Chart(...)` call in the app gets a subtle band over the most recent point and that point's value labelled in the dataset's own colour, with no per-chart wiring. A chart can opt out via `options.mhTodayHighlight:false` / `options.mhDataLabels:false`. Only the *last real* point per dataset is labelled, deliberately - full-history charts can have far more points than Zepp's fixed 7-day view, so labelling everything would be noise.
- A third, simpler layer sits inside the compare-metrics legend itself — individual metric cards within that one section, reordered/hidden via `sortByMMLegendOrder`/`getMMLegendHidden` (`mh_mm_legend_order` / `mh_mm_legend_hidden`), scoped to whatever's currently selected rather than a fixed key list.

**Real lockout bug found in this same phase, now a standing rule for any future hide-capable UI**: the ▲▼👁 controls originally lived inside each section/card's own header — hiding a section hid its own restore control with it, no way back short of clearing localStorage. Every hide-capable group now ships with a restore strip rendered *outside* every hideable element (`mmSectionRestoreStrip`, `trendCardsRestoreStrip`), showing a chip per hidden item regardless of hidden state. Any new hide toggle added to this app should follow the same rule: the control to undo a hide can never live entirely inside the thing being hidden.
...
`mergeCategoriesIntoLibraryItem(libItem, cats)` — the single shared write path for adding categories to an existing library item: seeds `libItem.categories` from `getItemCategories(libItem)` if empty, then only ever pushes non-duplicates, never replaces the array wholesale. Used by `addItemCategory()` (Library edit form), `saveLibraryOnlyFromPreview()` (chat "Save only"), and `confirmFuzzyLibOverwrite()` (fuzzy-match confirm) — the latter two used to each do a full `lib[idx] = newEntry` replace instead, which silently dropped a library item's existing categories any time the replacement object didn't itself carry a `.categories` array (i.e. whenever "Categories for Save only" was left blank, the normal case when just re-saving updated macros). Both now update the target's other fields in place and route any newly-typed categories through this shared function instead.

`renderCategoryChipsHtml(origIdx, categories)` — shared chip markup for a library item's category list, used both by `renderLibrary()`'s initial render and by `addItemCategory()`/`removeItemCategory()`, which patch only `#lib-edit-cats-${origIdx}` in place after a chip add/remove rather than calling `renderLibrary()`. That full re-render used to rebuild the entire library list on every single chip edit, which wiped the `.open` CSS class keeping that item's edit form expanded (`.library-edit-form.open { display: flex; }`) — editing several categories in a row silently bounced back to the collapsed card list after each tap.

`_saveLibraryOnlyFromPreviewImpl()`'s per-food categories are the union of two sources, not just one: the shared bubble-level "Categories for Save only" field (`sharedCats`, applies to every food in the preview) and each food's own `f._categoriesOverride` (typed into the ✏️ per-item edit form's CATEGORIES field, set by `applyPreviewEdit()`, survives the bubble's re-renders since it lives on the food object itself, not the DOM). For a brand-new item with neither field filled in, `.categories` is now seeded explicitly from `getItemCategories(entry)` at creation time rather than left unset — previously auto-detection only ever covered what got *displayed*, with nothing actually persisted on the item until a manual edit forced it.
...
## Condition-aware ketosis wording

`isKetogenicCondition()` — the single source of truth for whether the current `mh_condition` is genuinely a ketogenic protocol. Deliberately reuses `CONDITION_META[cond].protocol`'s own text (`.toLowerCase().includes('ketogenic')`) rather than a second hardcoded condition list — the two would otherwise need to be kept in sync by hand, and at least one pre-existing ad-hoc check (`isGBMorEpilepsy`, several inconsistent variants of which existed across different files scanned this session) had already drifted, one covering only 2 of the 5 real ketogenic conditions (gbm, epilepsy, strict_keto, migraine, cluster_headache).

Consumers found and fixed this session: the main Today-tab badge and its tap-through "Daily Carb Zones" tooltip (`updateCarbZonesCondition()`, which also swaps the GKI note's content or hides it entirely for non-keto conditions, and the Log-tab "Ketosis check" chip's *displayed* label specifically — the chip's `onclick` still sends the literal phrase "ketosis check" unconditionally, since `getKetosisStatus()`'s own intent-detection keys off that exact word appearing in the message; changing what's sent would have broken that detection), `getKetosisStatus()` itself (every message branch, not just the one line that previously checked condition), the streak-milestone celebration text (`STREAK_MILESTONES_KETO` / `STREAK_MILESTONES_GENERAL`, selected via a `Proxy` at read time — same pattern already used for `TARGETS`), `updateFatTargetAdvice()` (new, replacing 3 hand-duplicated copies of the same Settings fat-target advice), the dashboard's fat-ratio smart-alert (`renderKetosisSmartAlert` — hides entirely rather than reframes, since a 65% fat floor isn't a meaningful concept for a non-ketogenic protocol at all), several Trends chart labels (`getDrillConfig('ketosis')`'s title, the carbs-drill panel's section title/insights, the Ketosis & Weight Correlation heading), and the "What can I ask?" help response.

## Gross-vs-net carbs consistency

`getEffectiveCarbs(totals)` / `getEffectiveCarbsForDay(day)` (net-of-polyols carbs when `mh_carb_method` is `'net'`, falling back to gross for historical days predating fibre/polyols tracking) existed correctly before this session — the bug was never in these functions themselves, but in roughly 20 separate call sites across the app that read `day.totals.carbs`/`totals?.carbs` directly instead of going through them, for real adherence percentages and averages, not just display text.

Found via a specific report ("carbs have always been around 8 or 9, what's changed") that traced to `buildMealImpactHtml`'s Carbs row comparing a gross "before" against a net "after" — genuinely impossible arithmetic (a total appearing to *decrease* after adding a carb-containing item) that made the underlying pattern visible. Once found, a full grep sweep (`\.totals\.carbs\b|totals?\.carbs\b`, manually triaged against `getEffectiveCarbs`/known-safe patterns) turned up the remaining ~19 instances across `renderReportSummary`, Weekly Summary, the printable clinical report, `generateAIBrief`, the AI daily-insight prompt (which also had a second bug — a flat always-`targets.standard` ceiling, ignoring Occasion/Holiday days' own higher ceiling), Sleep-vs-Ketosis correlation, several Trends cards and the Compare Metrics "Carbs" overlay, a Period Comparison table, a carb-tier distribution breakdown, `getCarbOverageAddressedStatus` (feeds the weight-chart's addressed/unaddressed color), a day-detail panel, a per-day AI-report breakdown, the weekly push notification, and the carer-link export.

**A specific failure mode worth naming**: four of these (Sleep-Ketosis, the AI daily-insight, Reports summary, `generateAIBrief`) had an adherence-*percentage* calculation already correctly using `getEffectiveCarbs`, sitting right next to an "avg carbs" figure in the same function that was still raw gross — an earlier fix had corrected the pass/fail comparison but not its sibling average, and reading the comment claiming "same fix as X" was not sufficient evidence that X's own arithmetic (as opposed to its label) was actually correct. Two instances were confirmed correctly gross by design and left alone: the calorie-macro-split donut chart (about total caloric contribution, not ketosis adherence) and a day-rollover deduplication equality check (raw-to-raw, where a display preference is irrelevant to whether two stored values match).

## Saved Prompts scope tagging

Each saved prompt now carries an optional `scope` field: `'today'`, `'history'`, `'both'`, or absent (unclassified — treated as matching everywhere, so existing prompts never silently vanish from a list they used to appear in). `renderSavedPromptsManagerList()` filters by `_savedPromptsManagerRunFn` (which tab's Browse button opened the manager) against this field, with a "Show all prompts (N hidden)" override link and a 🪄 batch auto-classify action (same keyword logic as the live suggester, run once across every currently-unclassified prompt) that only appears when at least one unclassified prompt exists.

`_suggestSavedPromptScope()` — plain keyword matching against the question text as it's typed (`_SAVED_PROMPT_TODAY_WORDS` / `_SAVED_PROMPT_HISTORY_WORDS`), not an AI call, since it needs to be instant and a wrong guess costs nothing to correct. Backs off permanently the moment `_savedPromptScopeManuallySet` is set (any direct tap on a toggle button) for the rest of that form session, so it only ever offers a first guess and never fights a deliberate choice; editing an *existing* prompt's text treats its already-stored scope as equivalent to a manual choice, so a minor text edit doesn't retroactively override a scope set before this feature existed a second time.

This exists because `state.history` only ever contains completed days — today gets appended at midnight rollover, never before — so the Log tab (only ever sees `state.todayLog`) and the pre-existing Reports pipeline (only ever saw `state.history`) were each structurally incapable of answering a question needing the other's data, regardless of which list a saved prompt happened to be tapped from. New `_historyIncludingToday()` builds a synthetic "today" entry from `calcTotals(state.todayLog)` (wearable fields intentionally omitted — genuinely unknown for a day still in progress, same as a day with no wearable sync) and merges it into whatever `state.history` produces, making "History" scope a true superset. Wired into the two general-purpose Reports AI call sites (`askReportsAI`, the oncology narrative); `generateGBMSummary()` uses a separate, date-range-respecting history function shared with the Trends charts, so today is merged in there conditionally — only when it actually falls inside whatever range is currently selected — rather than forcing that function to always ignore the range picker.

## Portion parsing centralization

`_parsePortionGrams(str)` — parses a dedicated portion/amount field (expected to *be* a number, not buried in a longer sentence) into grams: accepts an explicit unit ("15g", "100ml") or a bare number ("15"), the latter unambiguously meaning grams in a portion-size context. Root cause of a real, reproducible bug (a library item's stored portion of "15" — no unit — silently failing to scale, showing identical macros whether 15g or 100g was entered): the strict-unit-only regex this eventually replaced (`/^\d+(\.\d+)?\s*(g|ml|kg|l)\b/i`, no bare-number fallback) had been independently hand-copied across 8 separate call sites — `showLibraryPortionSelector`, `scaleLibraryPortion`, `logLibraryWithPortion`, recipe-import matching, both voice-add flows (recipe ingredient and library item), the shared `_libItemToScalableIngredient` helper (itself used by recipe-substitute and suggested-combo items), and Refresh from Library — each one failing the same way on a bare-number portion, falling back to a 100g guess and never actually scaling regardless of the amount entered.

Deliberately distinct from `_extractGramsFromAmount(str)`, which searches for a unit-suffixed number *anywhere* in a longer free-text string (e.g. "1 can, 440ml") — that one has no bare-number fallback on purpose, since a bare number floating in a longer sentence isn't unambiguous the same way one is when it's the entire contents of a dedicated portion field.
